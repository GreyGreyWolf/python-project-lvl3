...
def write_content(data, path):
    try:
        ...
        engine.app_log.debug(sys.exc_info()[:2])
        engine.app_log.error('Not enough space to write the file')
        raise engine.PageLoaderException() from e
    engine.app_log.info(f'Data is written to {path}!')
